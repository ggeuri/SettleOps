/* =========================================================
   SettleOps - Flyway V1 Init (MySQL 8.x)  [DEV FRIENDLY]
   - DROP 없음 (Flyway는 누적 마이그레이션)
   - FK는 OFF (통합 직전 V99에서 ON)
   - CHECK는 최소화(개발/테스트 발목 방지) → 필요 시 V99에서 강화(주석 템플릿 제공)
   - meta_json(JSON)은 DEV 편의상 NULL 허용 → 추후 NOT NULL로 강화(주석 템플릿 제공)
...
CREATE TABLE IF NOT EXISTS payment (
  payment_id        VARCHAR(32) NOT NULL COMMENT '결제 고유 ID (예: PAY_1001)',
  order_id          VARCHAR(32) NOT NULL COMMENT '주문 ID(seed 식별자). 멱등 SoT는 idempotency_record',
  ...
  -- [IMPORTANT] order_id UNIQUE 제거(주문=결제 1개 강제 방지). 멱등은 idempotency_record가 SoT.
  ...
  COMMENT='결제 SoT (운영/증빙 관점: DELETE 금지 권장, 서비스 레벨에서 차단)';

-- =========================================================
-- 2) PAYMENT_EVENT (insert-only)
-- =========================================================
CREATE TABLE IF NOT EXISTS payment_event (
  event_id     VARCHAR(36) NOT NULL COMMENT '이벤트 고유 ID (UUID)',
  payment_id   VARCHAR(32) NOT NULL COMMENT 'FK -> payment.payment_id (V99에서 FK ON)',
  ...
  -- [MVP] 부분승인/부분캡처 없음 전제 → 각 payment 당 각 event_type 1회만 허용
  UNIQUE KEY uk_payment_event_payment_type (payment_id, event_type),

  KEY idx_payment_event_payment_id (payment_id),
  KEY idx_payment_event_request_id (request_id),
  KEY idx_payment_event_type_occurred (event_type, occurred_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
  COMMENT='결제 이벤트(insert-only). 증빙 성격이므로 payment 삭제 시 cascade 금지(RESTRICT 권장)';
...
CREATE TABLE IF NOT EXISTS idempotency_record (
  ...
  payment_id        VARCHAR(32)   NULL COMMENT '결과 리소스(성공 시 paymentId)',
...
CREATE TABLE IF NOT EXISTS settlement_batch (
  ...
  created_at    DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '배치 시작/기록 시각',
  finished_at   DATETIME    NULL COMMENT '[운영] 배치 종료 시각(옵션)',
...
CREATE TABLE IF NOT EXISTS settlement (
  settlement_id        VARCHAR(36) NOT NULL COMMENT '정산 ID (UUID 권장)',
  settlement_no        VARCHAR(50) NOT NULL COMMENT '표시용 정산 번호(예: SET-YYYYMMDD-001)',
  batch_id             BIGINT      NOT NULL COMMENT 'FK -> settlement_batch.batch_id (V99에서 FK ON)',
  ...
  paid_requested_at    DATETIME    NULL COMMENT '지급 요청 시각',
  paid_approved_by     VARCHAR(32) NULL COMMENT '지급 승인자(4-eyes)',
  paid_approved_at     DATETIME    NULL COMMENT '지급 승인 시각',
...
CREATE TABLE IF NOT EXISTS settlement_line (
  ...
  settlement_id       VARCHAR(36) NOT NULL COMMENT 'FK -> settlement.settlement_id (V99에서 FK ON)',
  payment_id          VARCHAR(32) NOT NULL COMMENT 'FK -> payment.payment_id (V99에서 FK ON)',
...
CREATE TABLE IF NOT EXISTS hold (
  hold_id               VARCHAR(36) NOT NULL COMMENT 'UUID',
  settlement_id         VARCHAR(36) NOT NULL COMMENT 'FK -> settlement.settlement_id (V99에서 FK ON)',
...
CREATE TABLE IF NOT EXISTS hold_event (
  ...
  hold_id       VARCHAR(36) NOT NULL COMMENT 'FK -> hold.hold_id (V99에서 FK ON)',
  event_type    VARCHAR(32) NOT NULL,
  request_id    CHAR(36)    NOT NULL,
  actor_type    VARCHAR(32) NOT NULL,
  actor_id      VARCHAR(32) NOT NULL,
  occurred_at   DATETIME    NOT NULL,
  meta_json     JSON        NULL COMMENT '[DEV] NULL 허용. 추후 NOT NULL + DEFAULT {} 규칙 도입 시 강화',
...
CREATE TABLE IF NOT EXISTS refund (
  refund_id     VARCHAR(32)  NOT NULL COMMENT '환불 ID (MVP: RFD_**** 등)',
  payment_id    VARCHAR(32)  NOT NULL COMMENT 'FK -> payment.payment_id (V99에서 FK ON)',
  ...
  currency      VARCHAR(8)   NOT NULL DEFAULT 'KRW' COMMENT '통화 (MVP: KRW)',

  status        VARCHAR(64)  NOT NULL COMMENT 'REQUESTED/APPROVED/REJECTED',
  reason_text   VARCHAR(255) NOT NULL COMMENT '환불 사유(텍스트, MVP 필수)',

  requested_at  DATETIME     NOT NULL COMMENT '업무상 요청 발생 시각',
  decided_at    DATETIME     NULL COMMENT '승인/거절 시각(결정 전 NULL)',

  created_at    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'row 생성 시각',
  updated_at    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'row 갱신 시각',

  PRIMARY KEY (refund_id),

  -- [LOCKED/MVP] payment_id당 환불 1회만 가능(기획서 합의)
  ...
  COMMENT='환불 SoT(최종 상태). 주체/코멘트/이벤트는 refund_event/audit_log에서 조회';

-- =========================================================
-- 10) REFUND_EVENT (insert-only)
-- =========================================================
CREATE TABLE IF NOT EXISTS refund_event (
  refund_event_id VARCHAR(32) NOT NULL,
  refund_id       VARCHAR(32) NOT NULL COMMENT 'FK -> refund.refund_id (V99에서 FK ON)',
...
CREATE TABLE IF NOT EXISTS refund_settlement_link (
  refund_settlement_link_id VARCHAR(32) NOT NULL,
  refund_id     VARCHAR(32) NOT NULL COMMENT 'FK -> refund.refund_id (V99에서 FK ON)',
  settlement_id VARCHAR(36) NOT NULL COMMENT 'FK -> settlement.settlement_id (V99에서 FK ON)',
  created_at    DATETIME    NOT NULL,

  PRIMARY KEY (refund_settlement_link_id),

  UNIQUE KEY uk_rsl_refund (refund_id),
  KEY idx_rsl_settlement (settlement_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
  COMMENT='환불이 특정 settlement에 반영되었다는 증거(insert-only)';
...
-- =========================================================
-- [DEV NOTE] CHECK/JSON 강화 템플릿은 V99__fk_on.sql에 모아둠
-- =========================================================
